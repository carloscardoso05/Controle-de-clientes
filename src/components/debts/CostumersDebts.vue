<template>
	<div class="rounded-md max-w-3xl mx-auto space-y-6 border shadow-inner py-4">
		<div class="xs:mx-8 mx-2 mt-2 border-b border-neutral-400 flex justify-between">
			...
			<h2 class="shrink-0 max-w-fit text-right tabular-nums text-base xs:text-lg">{{ formatMoney(costumerData.totalDebt) }}</h2>
		</div>
		...
		<div class="space-y-6 text-left xs:mx-10 mx-4 grid-cols-">
			<div class="flex place-content-between space-x-8 border-l-2 border-emerald-400" v-for="debt in costumerDebts" :key="debt.id">
				<div class="grow ml-4">
					<div>
						<p class="text-xs xs:text-sm font-light">{{ getDateString((debt.dateTime.toJSON() as any)["seconds"] * 1000) }}</p>
						<p class="text-lg xs:text-xl font-medium">{{ capitalize(debt.description) }}</p>
					</div>
					<p class="text-sm xs:text-base align-bottom tabular-nums underline decoration-solid decoration-emerald-500">{{ formatMoney(debt.price) }}</p>
				</div>
